...
contract FundRaise {
    ...
    function createEvent(string memory title, string memory decription, uint goal) public{
        uint idForNewEvent = eventId;
        address eventCreator = msg.sender;
        FundRaiseEvent memory newFundRaise = FundRaiseEvent(title, decription, goal,0,idForNewEvent,true, eventCreator);
